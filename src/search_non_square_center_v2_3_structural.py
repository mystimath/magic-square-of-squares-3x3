#!/usr/bin/env python3
"""
Branche B2 — v2.3 STRUCTURAL/ HYBRIDE
=================================
...
def pair_square_count(center: int, offset: int) -> int:
    o = abs(offset)
    return int(is_square(center - o)) + int(is_square(center + o))


def candidate_structural7(center: int, p: int, q: int) -> Candidate:
    vals = (
        center - p + q,
        center + 2 * p - q,
        center - p,
    ...
    return make_candidate("v2.3_structural7", center, p, q, vals)
...
def recombine_center_offsets(
    ...
    # Exact8 conservé pour compatibilité éventuelle.
    if search_mode in ("exact8", "both"):
        if len(off) < 4:
    ...
    # v2.3 : relaxed7 structurel.
    #
    # On ne teste plus tous les couples (p, q).
    # On ne garde que les couples où au moins une paire bonus est déjà
    # une paire opposée complète autour du centre :
    #
    #   r = p - q      pour (a, i)
    #   s = 2p - q    pour (b, h)
    #
    # Si aucun des deux offsets r ou s n'est dans off_set, le couple
    # ne peut pas produire un 7/9 avec centre non carré.
    if search_mode in ("relaxed7", "both"):
        if len(off) < 3:
            stats["centers_too_few_offsets_structural7"] += 1
        else:

            def try_pair(p: int, q: int, reason: str) -> bool:
                if q <= 0 or q >= p:
                    return False
                ...
                r_complete = r in off_set
                s_complete = s in off_set

                if not r_complete and not s_complete:
                    stats["reject_no_complete_bonus_pair"] += 1
                    return False

                # Carrés garantis : c,g,d,f = 4.
                # Le centre peut compter seulement si center-mode square/any.
                guaranteed = 4 + int(is_square(center))

                # Bonus possibles dans les paires (a,i) et (b,h).
                bonus = pair_square_count(center, r) + pair_square_count(center, s)

                if guaranteed + bonus < min_total:
                    stats["reject_structural_bonus_too_low"] += 1
                    return False

                cand = candidate_structural7(center, p, q)

                if not candidate_ok(cand, min_total, primitive_only, distinct_roots):
                    stats["reject_candidate_structural7"] += 1
                    return False
                ...
                seen_keys.add(key)
                out.append(cand)
                stats[f"candidates_kept_{reason}"] += 1
                stats["candidates_kept_structural7"] += 1
            ...
            # Cas 1 : la paire (a, i) est complète.
            # Donc r = p - q est dans off_set.
            for p in off:
                for r in off:
                    q = p - r
                    if q in off_set:
                        stop = try_pair(p, q, "complete_ai")
                        if stop:
                            return out, stats

            # Cas 2 : la paire (b, h) est complète.
            # Donc s = 2p - q est dans off_set.
            for p in off:
                for s in off:
                    q = 2 * p - s
                    if q in off_set:
                        stop = try_pair(p, q, "complete_bh")
                        if stop:
                            return out, stats
